...
```js
var greatheroes = mquery(movieCollection).where('rating').gte(4.5).toConstructor();

// use it!
greatheroes().count(function (err, n) {
  console.log('There are %d great heroes', n);
});

greatheroes().where({ name: /^Life/ }).select('name').find(function (err, docs) {
  console.log(docs);
});
```
